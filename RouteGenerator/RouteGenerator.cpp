...
void RouteGenerator::sceneStatic() {
    ...
    float Pipe1_data[12] = { -1, 4, 0, 1, 4, 0, 1, 2, 0, -1, 2, 0 };
    const float Pipe2_data[12] = { 2,4,0,4,4,0,4,2,0,2,2,0 };
    const float Pipe3_data[12] = { -4, 1,0,-2,1,0,-2,-1,0,-1,-1,0 };
    const float Pipe4_data[12] = { -1,1,0,1,1,0,1,-1,0,-1,-1,0 };
    const float Pipe5_data[12] = { -4,-2,0,-2,-2,0,-2,-4,0,-4,-4,0 };
    const float Pipe6_data[12] = { 2,-2,0,4,-2,0,4,-4,0,2,-4,0 };
    for (int i = 0; i < sizeof(Pipe1_data) - 3; i++) {
        PipeVertices.push_back(Pipe1_data[i]);
        PipeColor.push_back(1.0);
        mVertices.push_back(Pipe1_data[i]);
        mColors.push_back(1.0);
    }
    Pipe Pipe1(PipeVertices, PipeColor);
    PipeVertices.clear();
    PipeColor.clear();
    for (int i = 0; i < sizeof(Pipe2_data); i++) {
        PipeVertices.push_back(Pipe2_data[i]);
        PipeColor.push_back(1.0);
        // mVertices.push_back(Pipe2_data[i]);
         //mColors.push_back(1.0);
    }
    Pipe Pipe2(PipeVertices, PipeColor);
    PipeVertices.clear();
    PipeColor.clear();
    for (int i = 0; i < sizeof(Pipe3_data); i++) {
        PipeVertices.push_back(Pipe3_data[i]);
        PipeColor.push_back(1.0);
        mVertices.push_back(Pipe3_data[i]);
        mColors.push_back(1.0);
    }
    Pipe Pipe3(PipeVertices, PipeColor);
    PipeVertices.clear();
    PipeColor.clear();
    for (int i = 0; i < sizeof(Pipe4_data); i++) {
        PipeVertices.push_back(Pipe4_data[i]);
        PipeColor.push_back(1.0);
        // mVertices.push_back(Pipe4_data[i]);
        // mColors.push_back(1.0);
    }
    Pipe Pipe4(PipeVertices, PipeColor);
    PipeVertices.clear();
    PipeColor.clear();
    for (int i = 0; i < sizeof(Pipe5_data); i++) {
        PipeVertices.push_back(Pipe5_data[i]);
        PipeColor.push_back(1.0);
        // mVertices.push_back(Pipe5_data[i]);
        // mColors.push_back(1.0);
    }
    Pipe Pipe5(PipeVertices, PipeColor);
    PipeVertices.clear();
    PipeColor.clear();
    for (int i = 0; i < sizeof(Pipe6_data); i++) {
        PipeVertices.push_back(Pipe6_data[i]);
        PipeColor.push_back(1.0);
        //mVertices.push_back(Pipe6_data[i]);
        //mColors.push_back(1.0);
    }
    Pipe Pipe6(PipeVertices, PipeColor);
    PipeVertices.clear();
    PipeColor.clear();
...
void RouteGenerator::readOBJ() {
    ...
    std::string filePath = "D:\\Ankit_Workspace\\Pipe.obj";
    ...
    for (int i = 0; i < mPoints.size(); i+=1) {
        mVertices << mPoints[i].X() << mPoints[i].Y() << mPoints[i].Z();
    }
...
void RouteGenerator::LeftMove() {
    ...
    translatedPoints = tf.translate_X_Left(mPoints);
    mPoints = translatedPoints;

    for (int i = 0; i < translatedPoints.size(); i++) {
        // Apply scaling to each coordinate
        mVertices << scaleFactor * translatedPoints[i].X() << scaleFactor * translatedPoints[i].Y() << scaleFactor * translatedPoints[i].Z();
    }
